...
class RidePaymentManager
{
    public function payment($ride, $paymentType)
    {
        ...
        if ($paymentType ==  Status::PAYMENT_TYPE_GATEWAY) {
            ...
            if ($ride->tips_amount > 0) {

                $user->balance -= $ride->tips_amount;
                $user->save();

                $transaction               = new Transaction();
                $transaction->user_id      = $user->id;
                $transaction->amount       = $ride->tips_amount;
                $transaction->post_balance = $user->balance;
                $transaction->charge       = 0;
                $transaction->trx_type     = '-';
                $transaction->trx          = $ride->uid;
                $transaction->remark       = 'driver_tips';
                $transaction->details      = 'Tips given for ride UID ' . $ride->uid . ' amount ' . showAmount($ride->tips_amount);
                $transaction->save();
            }
        }
        ...
        if ($paymentType ==  Status::PAYMENT_TYPE_GATEWAY) {
            ...
            if ($ride->tips_amount > 0) {
                ...
                $driver->balance += $ride->tips_amount;
                $driver->save();
                ...
                $transaction->trx          = $ride->uid;
                $transaction->remark       = 'driver_tips';
                $transaction->details      = 'Received tips from ride UID ' . $ride->uid . ' amount ' . showAmount($ride->tips_amount);
                $transaction->save();
            }
        }
