...
function apiResponse(string $remark, string $status, mixed $message = [], array $data = [], $statusCode = 200): JsonResponse
{
    $response = [
        'remark' => $remark,
        'status' => $status
...
function getRideMiniumAmount($serviceMinimumAmount)
{
    return max((gs('ride_minimum_amount') ? gs('ride_minimum_amount') : 0), $serviceMinimumAmount);
}


function formatRating($rating)
{
    return round(max(0, min(5, $rating)) * 2) / 2;
}

function generateRatingStar(float $rating)
{
...
function returnServiceResponse($returnType, mixed $message, array $restOfTheData = [])
{
    return [
        'success'   => $returnType,
        'message' => $message,
...
function notifyBack(String $type, String $message)
{
    $notify[] = [$type, $message];
    return back()->withNotify($notify);
}

function serviceSuccess(string $remark, string $message, array $data = [])
{
    return array_merge([
        'remark'  => $remark,
        'status'  => 'success',
...
function serviceError(string $remark, string $message)
{
    return [
        'remark' => $remark,
        'status' => 'error',
...
function googleFormatResponse(mixed $data)
{
