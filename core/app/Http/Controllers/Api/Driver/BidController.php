...
class BidController extends Controller
{
    public function create(Request $request, $id)
    {
        ...
        $data['driver_image']       = getImage(getFilePath('driver') .'/'. $driver->image, getFileSize('driver'), true);
        $data['brand_image']        = getImage(getFilePath('brand') . optional($driver->vehicle->brand)->image, getFileSize('brand'));
    ...
    {
        $bids = Bid::with('driver')->where('driver_id', auth()->id())->where('status', Status::BID_PENDING)->orderBy('id', 'desc')->paginate();
        
        return apiResponse("bids", "success", [], [
            'bids' => $bids
        ]);
    }
