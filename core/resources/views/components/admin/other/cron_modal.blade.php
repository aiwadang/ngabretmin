    ...
    <x-admin.ui.modal.body class="p-3 p-md-4">
        <div class="mb-3">
            <div class="alert alert--warning fw-bold">
                @lang('Please confirm that your server’s PHP execution time is set to more than 1 second. If it is lower, the cron jobs may not run properly, and drivers may fail to receive ride notifications on time.')
            </div>
        </div>
        <div class="mb-4">
            <div class="justify-content-between d-flex flex-wrap mb-2">
                <label class="fs-14">@lang('Global Cron Command')</label>
        ...
        <div class="mb-4">
            <div class="justify-content-between d-flex flex-wrap mb-2">
                <label class="fs-14">@lang('Instant Ride Trigger Cron Command')</label>
                <small class="fst-italic">
                    @lang('This cron job must be set to run every minute.')
                </small>
            </div>
            <div class="input-group input--group">
                <input type="text" class="form-control" value="curl -s {{ route('cron') }}?alias=ride_queue"
                    readonly>
                <span class="input-group-text cursor-pointer copyBtn"
                    data-copy="curl -s {{ route('cron') }}?alias=ride_queue">
                    <i class="fas fa-copy me-1"></i>@lang('Copy')</span>
                </span>
            </div>
        </div>
