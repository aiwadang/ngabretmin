@auth
    @php
        $layout = $activeTemplate . 'layouts.auth';
        $extend = 'auth';
    @endphp
@else
    @php
                            ...
                            @auth
                                <div class="d-flex justify-content-between flex-wrap gap-2 ">
                                    @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->user)
                                        <button class="btn btn--danger close-button  confirmationBtn" type="button"
                                            data-question="@lang('Are you sure to close this ticket?')"
                                            data-action="{{ route('ticket.close', $myTicket->id) }}"><i
                                                class="fas fa-lg fa-times-circle"></i> @lang('Close Ticket')
                                        </button>
                                    @endif
                                    <a href="{{ route('ticket.index') }}" class="btn  btn--base">@lang('My Support Ticket')</a>
                                </div>
                            @endauth
                        </div>
                        <div class="card-body">
                    ...
                    <div class="col-md-4  removeFileInput">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <button type="button" class="input-group-text removeFile bg--danger border--danger text-white h-49"><i class="fas fa-times"></i></button>
                            </div>
                        </div>
                    </div>
                `)
