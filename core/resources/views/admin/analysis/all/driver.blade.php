...
@section('panel')
    <div class="row responsive-row">
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment') }}" :currency="true" variant="info"
                title="Total Earning" :value="$widget['total_commission_paid']" icon="las la-file-invoice-dollar" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.driver.commission') }}" :currency="true" variant="primary"
                title="Total Commission Paid" :value="$widget['total_commission_paid']" icon="las la-coins" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.rides.tips') }}" :currency="true" variant="warning"
                title="Total Tips Earning" :value="$widget['total_commission_paid']" icon="las la-search-dollar" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment') }}" :currency="true" variant="success"
                title="Average Earning Per Ride" :value="$widget['total_commission_paid']" icon="las la-money-bill-alt" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment.today') }}" :currency="true"
                variant="success" title="Today Earning" :value="$widget['total_commission_paid']" icon="las la-calendar" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment.week') }}" :currency="true"
                variant="warning" title="This Week Earning" :value="$widget['total_commission_paid']" icon="las la-calendar" />
        </div>

        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment.month') }}" :currency="true"
                variant="primary" title="This Month Earning" :value="$widget['total_commission_paid']" icon="las la-calendar" />
        </div>

        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.report.rider.payment.year') }}" :currency="true" variant="info"
                title="This Year Earning" :value="$widget['total_commission_paid']" icon="las la-calendar" />
        </div>
                        ...
                        <x-admin.ui.table>
                            ...
                            <x-admin.ui.table.body>
                                @forelse((@$payments ?? []) as $payment)
                                    <tr>
                                        <td>
                                            <a target="_blank" href="{{ route('admin.rides.detail', $payment->ride_id) }}">
                                                {{ @$payment->ride->uid }}
                                            </a>
                                        </td>
                                        <td>
    ...
    <script>
        ...
        (function($) {
            ...
            const spentChart = (startDate, endDate) => {
                const url = @json(route('admin.analysis.driver.chart.spent', '0'));
